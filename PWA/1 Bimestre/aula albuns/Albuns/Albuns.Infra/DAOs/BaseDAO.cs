...
namespace Albuns.Infra.DAOs
{
    public abstract class BaseDAO<T> where T: Modelo
    {
        ...
        {
            var sql = $"insert into {NomeTabela} ({GetNomeCamposParaInsert()}) values ({GetNomeParametrosParaInsert()})";
        ...
        private string GetNomeCamposParaInsert()
        {
            var sb = new StringBuilder();

            sb.Append(NomeCamposExcetoId[0]);
        ...
        private string GetNomeParametrosParaInsert()
        {
            var sb = new StringBuilder();
